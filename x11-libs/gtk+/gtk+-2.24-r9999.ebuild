...
src_unpack() {
	...
	IFS=".-_"
	for x in ${PV}; do
		if [[ -z "$GTKMAJORVER" ]]; then
			GTKMAJORVER=$x
			else if [[ -z "$GTKMINORVER" ]]; then
				GTKMINORVER=$x
			fi;
		fi
	done
	IFS="$IFSBACKUP"
	GTKREVISION=${PR}
	EGIT_REPO_URI="file:///home/rainman/projects/gnome.org/${PN}/.git"
	EGIT_BRANCH="gtk-${GTKMAJORVER}-${GTKMINORVER}"
...
src_prepare() {
	...
	strip_builddir SUBDIRS tutorial docs/Makefile.am
	strip_builddir SUBDIRS faq docs/Makefile.am
	...
	if ! use test; then
		...
		strip_builddir SUBDIRS tests gdk/Makefile.am gtk/Makefile.am
	else
